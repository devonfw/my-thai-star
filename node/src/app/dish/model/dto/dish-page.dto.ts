  ...
  static fromDishes(
    ...
    return plainToClass(DishPage, {
      ...
      content: dishes.map(dish => {
        return {
          dish:
            dish && plainToClass(Dish, dish, { excludeExtraneousValues: true }),
          image: dish.image && plainToClass(Image, dish.image),
          extras:
            dish.ingredients && plainToClass(Ingredient, dish.ingredients),
          categories:
            dish.categories && plainToClass(Category, dish.categories),
        };
      }),
    });
  }
