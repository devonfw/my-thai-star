...
=== Snack Service
Service created to serve as a factory of Angular Material Snack-bars, which are used commonly through the app. This service accepts some parameters to customize the snack-Bar and opens it with this parameters.
...
This directive located in the shared folder of user Area is used in 2 fields to make sure they have the same value. This directive is used in confirm password fields in register and change password.
...
=== Login
Login manages the user Area logic: login, register and change password.
