...
First we declare the component to be tested and a Fixture object, then, we configure the `testingModule` right in the same way we could configure the MenuModule with the difference here that tests always have to use the mock backend because we do not want to really depend on a server to test our components.
