 ...
 */
@Named
public class OrderedDishesPerMonthDaoImpl extends ApplicationDaoImpl<OrderedDishesPerMonthEntity>
    implements OrderedDishesPerMonthDao {
  /**
   * The constructor.
   */
  ...
  @Override
  public PaginatedListTo<OrderedDishesPerMonthEntity> findOrderedDishesPerMonth(
      OrderedDishesSearchCriteriaTo criteria) {
    ...
    // le,ge,lt,gt,eq,ne
    if (criteria.getStartBookingdate() != null) {
      Timestamp startBookingdate = criteria.getStartBookingdateTimestamp();
      query.where(Alias.$(orderedDish.getBookingdate()).after(startBookingdate));
    }

    if (criteria.getEndBookingdate() != null) {
      Timestamp endBookingdate = criteria.getEndBookingdateTimestamp();
